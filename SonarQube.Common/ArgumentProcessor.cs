...
namespace SonarQube.Common
{
    ...
    public static class ArgumentProcessor // was internal
    {
        ...
        private class ArgumentDescriptor
        {
			...
			/// <summary>
			/// Whether the argument is mandatory or not for a processing type
			/// </summary>
			public ProcessingType RequiredForType { get; set; } = ProcessingType.Unknown;
		...
		static ArgumentProcessor()
		{
			...
			Descriptors.Add(new ArgumentDescriptor()
				{ Id = KeywordIds.Type, Prefixes = new string[] { "/type:", "/t:" }, Required = true, Description = Resources.CmdLine_ArgDescription_ProccessingType });

			Descriptors.Add(new ArgumentDescriptor()
				{ Id = KeywordIds.ProjectKey, Prefixes = new string[] { "/key:", "/k:" }, RequiredForType = ProcessingType.Pre, Description = Resources.CmdLine_ArgDescription_ProjectKey });
            
            Descriptors.Add(new ArgumentDescriptor()
                { Id = KeywordIds.ProjectName, Prefixes = new string[] { "/name:", "/n:" }, RequiredForType = ProcessingType.Pre, Description = Resources.CmdLine_ArgDescription_ProjectName });
            
            Descriptors.Add(new ArgumentDescriptor()
                { Id = KeywordIds.ProjectVersion, Prefixes = new string[] { "/version:", "/v:" }, RequiredForType = ProcessingType.Pre, Description = Resources.CmdLine_ArgDescription_ProjectVersion });
            
            Descriptors.Add(new ArgumentDescriptor()
                { Id = KeywordIds.RunnerPropertiesPath, Prefixes = new string[] { "/runnerProperties:", "/r:" }, Description = Resources.CmdLine_ArgDescription_PropertiesPath });

			Descriptors.Add(new ArgumentDescriptor()
				{ Id = KeywordIds.Username, Prefixes = new string[] { "/username:", "/u:" }, Description = Resources.CmdLine_ArgDescription_Username });

			Descriptors.Add(new ArgumentDescriptor()
				{ Id = KeywordIds.Password, Prefixes = new string[] { "/password:", "/p:" }, Description = Resources.CmdLine_ArgDescription_Password });
        ...
        public static ProcessedArgs TryProcessArgs(string[] commandLineArgs, ILogger logger)
        {
            ...
            if(parsedOk && allRequiredExist)
            {
				...
				idToValueMap.TryGetValue(KeywordIds.Username, out username);
				idToValueMap.TryGetValue(KeywordIds.Password, out password);

                if (!string.IsNullOrEmpty(propertiesPath))
                {
                    processed = new ProcessedArgs(
						idToValueMap[KeywordIds.Type] == "pre" ? ProcessingType.Pre : ProcessingType.Post,
						idToValueMap.ContainsKey(KeywordIds.ProjectKey) ? idToValueMap[KeywordIds.ProjectKey] : null,
						idToValueMap.ContainsKey(KeywordIds.ProjectName) ? idToValueMap[KeywordIds.ProjectName] : null,
						idToValueMap.ContainsKey(KeywordIds.ProjectVersion) ? idToValueMap[KeywordIds.ProjectVersion] : null,
                        propertiesPath,
						username,
						password);
                }
		...
		private static bool TryGetProcessingType(IDictionary<KeywordIds, string> idToValueMap, ILogger logger, out ProcessingType type)
		{
			type = ProcessingType.Pre;
			var t = idToValueMap[KeywordIds.Type].ToLower();
			if (t == "pre")
			{
				return true;
			} else if (t == "post")
			{
				type = ProcessingType.Post;
                return true;
			}
			logger.LogError(Resources.ERROR_CmdLine_TypeIncorrect);
			return false;
        }
        ...
        private static bool CheckRequiredArgumentsSupplied(IDictionary<KeywordIds, string> idToValueMap, ILogger logger)
        {
			...
			{
				ProcessingType type;
				if (TryGetProcessingType(idToValueMap, logger, out type))
				{
					foreach(var desc in Descriptors.Where(d => d.RequiredForType != ProcessingType.Unknown && d.RequiredForType == type))
                    {
						bool exists = idToValueMap.ContainsKey(desc.Id) && !string.IsNullOrWhiteSpace(idToValueMap[desc.Id]);
						if (!exists)
						{
